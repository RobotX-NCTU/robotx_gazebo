...
void cluster_pointcloud()
{
  ...
  vg.setLeafSize (0.05f, 0.05f, 0.05f); //unit:cetimeter
  ...
  */
  ...
  int start_index = 0;
  int count = 0; //number of object in /obstacle_list 
  ...
  for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
  {
    ...
    if ((std::abs(ob_pose.x)-1.00 <= 0.0099) || (std::abs(ob_pose.y)-2.11 <= 0.005)){
      continue;
    }
    else{
      count += 1;
