...
namespace gazebo
{
  ...
  class BlinkVisualPluginPrivate
  {
    ...
    //  \brief Third color.
    public: common::Color colorC;
    //  \brief Fourth color.
    public: common::Color colorD;
...
void BlinkVisualPlugin::Load(rendering::VisualPtr _visual, sdf::ElementPtr _sdf)
{
  ...
  if (_sdf->HasElement("color_a")){
    cout << "Get ColorA" << endl;
    this->dataPtr->colorA = _sdf->Get<common::Color>("color_a");
  }

  // Get color B
  this->dataPtr->colorB.Set(0, 0, 1, 1);
  if (_sdf->HasElement("color_b")){
    cout << "Get ColorB" << endl;
    this->dataPtr->colorB = _sdf->Get<common::Color>("color_b");
  }

  this->dataPtr->colorC.Set(0, 1, 0, 1);
  if (_sdf->HasElement("color_c")){
    cout << "Get ColorC" << endl;
    this->dataPtr->colorC = _sdf->Get<common::Color>("color_c");
  }

  this->dataPtr->colorD.Set(0, 0, 0, 1);
  if (_sdf->HasElement("color_d")){
    cout << "Get ColorD" << endl;
    this->dataPtr->colorD = _sdf->Get<common::Color>("color_d");
  }
...
void BlinkVisualPlugin::Update()
{
  ...
  // Restart cycle
  if (elapsed >= (this->dataPtr->period*3.0+2.0))
    this->dataPtr->cycleStartTime = currentTime;
  ...
  // Color B -> C
  else if (elapsed >= this->dataPtr->period && elapsed <= this->dataPtr->period*2.0)
  {
    from = this->dataPtr->colorB;
    to = this->dataPtr->colorC;
    //elapsed -= this->dataPtr->period*0.5;
  }
  // Color C -> D
  else if (elapsed >= this->dataPtr->period*2.0 && elapsed <= this->dataPtr->period*3.0)
  {
    from = this->dataPtr->colorC;
    to = this->dataPtr->colorD;
    //elapsed -= this->dataPtr->period*0.5;
  }
  // Color D -> A
  else if (elapsed >= this->dataPtr->period*3.0 && elapsed < (this->dataPtr->period*3.0+2.0))
  {
    from = this->dataPtr->colorD;
    to = this->dataPtr->colorA;
    //elapsed -= this->dataPtr->period*0.5;
  }
  // Color A -> A
  else if (elapsed >= (this->dataPtr->period*3.0+2.0))
  {
    from = this->dataPtr->colorA;
    to = this->dataPtr->colorA;
    //elapsed -= this->dataPtr->period*0.5;
